...
def ct_to_expected_vp1_depth(
    ct: float, total_reads: int, config: SimConfig, vp1_length: int = 1700
) -> float:
...
def predict_completeness(
    ct: float, total_reads: int = 1_000_000, config: SimConfig | None = None
) -> str:
    if config is None:
        from nocasim.config import SimConfig
...
def viral_fragment_count(
    ct: float, target_vp1_depth: float, config: SimConfig, vp1_length: int = 1700
) -> int:
    vf = ct_to_viral_fraction(ct)
    on_target_rate = 1.0 - config.off_target_rate
    mean_frag_len = config.fragment_mean
